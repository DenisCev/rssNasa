...
class FeedParser: NSObject, XMLParserDelegate {
    ...
    private var parserCompletionHandler: (([RSS]) -> Void)?
    
    // 3
    func parseFeed(url: String, completionHandler: (([RSS]) -> Void)?) -> Void {
    ...
    {
        /*if elementName == "item" {
            let rssItem = RSS(title: currentTitle, description: currentDescription, pubDate: currentPubDate)
            rssItems += [rssItem]
        }*/
    }
}
